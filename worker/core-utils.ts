...
export abstract class IndexedEntity<S extends { id: string }> extends Entity<S> {
  ...
  static async create<TCtor extends CtorAny>(this: HS<TCtor>, env: Env, tenantId: string, state: IS<TCtor>): Promise<IS<TCtor>> {
    const id = this.keyOf(state);
    const inst = new this(env, id);
    await inst.save(state);
    const idx = new Index<string>(env, `${tenantId}:${this.indexName}`);
    await idx.add(id);
    return state;
  }
  ...
  static async ensureSeed<TCtor extends CtorAny>(this: HS<TCtor>, env: Env, tenantId: string): Promise<void> {
    const idx = new Index<string>(env, `${tenantId}:${this.indexName}`);
  ...
  static async delete<TCtor extends CtorAny>(this: HS<TCtor>, env: Env, tenantId: string, id: string): Promise<boolean> {
    const inst = new this(env, id);
  ...
  static async deleteMany<TCtor extends CtorAny>(this: HS<TCtor>, env: Env, tenantId: string, ids: string[]): Promise<number> {
  ...
  static async removeFromIndex<TCtor extends CtorAny>(this: HS<TCtor>, env: Env, tenantId: string, id: string): Promise<void> {
    const idx = new Index<string>(env, `${tenantId}:${this.indexName}`);
    await idx.remove(id);
  }
